...
public class E03CharBooleanType {
	...
	public static void main(String[] args) {
		 ...
		 * '(single quotation)으로 감싸서 펴현한다.
		 * 만약 "(double qutation)으로 감싸면 문자열로 인식하므로 string형으로 선언한다.
		 */
		char ch1 = '가';
		 ...
		 * EX)A => 65로 표현
		 *    a =>97로 표현
		 *    유니코드 : 1byte로 표현이 안되는 문자(한글, 한자 등)는 2byte가 필요하고, 이를 16진수로 정의한값을 말한다.
		 */
		...
		System.out.println("ch3(문자 출력)=>"+ ch3);//c출력
		System.out.println("ch3(아스키코드 출력)=>"+ (int)ch3);//67출력
		 ...
		 * 데이터형으로 산술연산(+,-,*,/)이나 비교연산(<,>= 등)
		 * 은 사용이 불가능하다. 단 논리연산은 가능하다.
		 */
		boolean bn1 = true;
		boolean bn2 = false;
