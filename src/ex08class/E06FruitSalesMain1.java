...
class FruitSeller
{
	...
	/*
	 * 클래스에서 상수로 선언하는 경우 반드시 선언과 동시에 초기화 해야한다 만약 선언과 초기화를 별도로 처리하면 에러가 발생한다 (테스트시
	 * 초기화되는 1000부분을 제거후 확인해볼것)
	 */

	// 판매자가 사과를 판매하는 행위를 표현한 멤버메소드
	/*
	 * 1.금액을 전달받는다. 2.금액에 해당하는 판매될 사과의 갯수를 구한다. 3.보유한 재고에서 갯수만큼 차감한다. 4.판매수입이 증가한다.
	 * 5.구매지에게 판매한 사과의 갯수를 반환한다.
	 */
...
class FruitBuyer
{
	...
	/*
	 * 1.판매자에게 금액을 지불한다 2.판매자가 금액을 해당하는 사과의 갯수를 반환해준다. 반환한 갯수만큼 보유한 사과의 갯수가 증가한다
