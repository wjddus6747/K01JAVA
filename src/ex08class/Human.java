...
public class Human
{
	...
	void walk() {
		//에너지를 1차감한후 음수인지 확인하여 음수이면 0으로 고정한다
		energy--;
		if(energy<0) {
			energy =0;
			...
			System.out.println("[walk]에너지가 1감소합니다");
		}
	}
	...
	void thinking() {
		...
		if(energy<0) {
			...
			System.out.println("[thinking]에너지가 2감소합니다");
		}
	}
	void eat() {
		//에너지를 2증가시칸후 10을 초과하면 10으로 고정한다
		energy+=2;
		if(energy>10) {
			energy =10;
			System.out.println("[eat]에너지는 10이최대입니다");
			System.out.println("0으로 고정됩니다");
		}
		else {
			System.out.println("[eat]에너지가 2증가합니다");
		}
	}
