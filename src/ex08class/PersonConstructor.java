...
public class PersonConstructor
{
	...
	/*
	 * 디폴트 생성자 : 해당 클레스에 생성자를 정의하지 않은 경우에 아래와 같은 메게변수, 실행부가 없는 생성자가 자동으로 삽입된다. 단
	 * 생성자를 하나라도 정의하면 다폴트 셍성자는 삽입되지 않는다(주의)
	 */
	...
	{ // 멤버변수와 메개변수를 구분하기 위해 this를 사용한다
		this.name = name;
		age = 1;
		addr = "출처분명";
	}
	...
	{/*
		 * this()는 생성자내에서 다른 생성자를 호출할때 사용한다 단 생성자내에서만 사용가능하고 멤버메소드에서는 사용이 불가능하다
		 */
		this(name, age, "미상");
		System.out.println("나는 인자 생성자[2] 입니다");
	}

	protected PersonConstructor(String _name, int age, String addr)
	{
		// 매개변수와 멤버면수명이 다르면 this를 쓰지 않아도 된다
		name = _name;
	...
	// 멤버매소드 : 멤버변수를 초기화 할 목적으로 정의한 메소드
	void initialize(String name, int age, String addr)
	{

		// 해당 코드는 생성자 내에서만 사용할수 있다.
		// this(name, age, "미상");
