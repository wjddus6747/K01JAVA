...
class Rectangle
{
	...
	public Rectangle(int ulx, int uly, int lrx, int lry)
	{
		...
		// 좌측좌표가 우측좌표가 뒤바뀐 경우 오류로 처리
		if (ulx >= lrx || uly >= lry)
		{
			System.out.println("좌/우측 죄표지정이 잘못되었습나다");
			isTrue = false;
		}
		// 멤버변수 초기화
		this.ulx = ulx;
	...
	public boolean isRange(int point)
	{
		if (point < 0 || point > 100)
			return false;// 0과 100사이의 밤위를 벗어나면 좌표오류
		else
			return true;
	}
	...
	public void showArea()
	{
		...
		if (isTrue == false)
		{
			System.out.println("죄표값 오류로 넓이계산아 불가합니다");
			return;
		}
		// 좌표값을 구하여 넓이 계산
		int xLength = lrx - ulx;
		int yLength = lry - uly;
		System.out.println("시긱형 넓이:" + (xLength * yLength));
	}
}
