...
public class StaticBlock
{
	...
	static
	{
		//static블럭안에서는 정적멤버인 staticVar애 접근가능
		staticVar = 1000;

		/*
		 * static 블럭안에서만 사용할수 있는 자역변수 선언
		...
		/*
		 * static 블럭내에서는 인스턴스형 멤버를 사용할수 없다.
		 */
		...
		//maim메소드보다 먼저 실행되는지 확인용 출력문
		System.out.println("=======static block===========");
	}
	//생성자 메소드
	/*
	 * 외부파일의 main메소드에서 실행되는 경우 해당 클래스의 객체가 
	 * 생성자를 통해 생성되기전에 static블럭이 먼저 실헹된다.
	 */
	public StaticBlock() {
		staticVar = -1;
	...
	{
		/*
		 * E02StaticBlockMain.java 에서 실행할때는 주걱처리후 실행하세요
		 * main메소드가 외부파일에 있을때 실행을 태스트하기 위함
		 */
		System.out.println("==메인메소드==");
		
		//loclaVar는 지역변수라 main메소드에서는 사용할수 없다.
		//System.out.println("localVar="+ localVar);
	}
}