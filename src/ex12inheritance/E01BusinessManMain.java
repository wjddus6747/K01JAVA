...
/*
 * 상속(Inheritance)
 * 	: 기존에 정으된 클래스에 메소드에 메소드와 변수를 추가하여 새로운 
 * 클래스를 정의하는것을 말한다
...
class Man{
	private String name;
	/*
	 * 만약 자식 클래스에서 Super()를 통해 매개변수를 전달하지 않는다면
	 * 초가화가 되지않아 null이 출력될 수 있다.
	 */
	public Man() {
...
class BusinessMan extends Man{
	...
	/*
	 * 상속받은 자식클래스에서는 부모클래스의 생성자를 호출하여 부모객체가 먼저 생성되도록 해야하는
	 * 책임을 가지고 있다.
	 * super()는 부모클래스의 생성자를 호출하는 역할을 하게 되며 만약 super()를 쓰지 않으면
	 * 디폴트생성자가 호출된다.
	 */
	public BusinessMan(String name, String company, String position) {
		super(name);/* 부모클레스의 매게변수가 1개인 생성자를 호출한다.
		*/
...
public class E01BusinessManMain
{
	...
	{
		BusinessMan man1 = new BusinessMan("이정연", "삼성", "회장");
		BusinessMan man2 = new BusinessMan("이준승", "누리시스템", "쫄따구");
