...
class Account{
	/*
	 * 멤버변숙가 상속관계에 있다 하더라도 private으로 선언되면 클래스 내부에서만 접근가능하다 
	 * 즉 ,상속박은 하위 클래스에서 직접 접근은 불가능하다.
	 */
	private int money;
	...
	/*
	 * 입금처리 : protected로 선언되었으로 상속관계에서 접근가능
	 */
	protected void depositMoney(int _money) {
		if(_money<0) {
			System.out.println("마이너스 금액은 입굼처리가 불가하다");
			return;
		}
		money += _money;
	}
...
class SavingAccount extends Account{
	 ...
	 * super()를 사용하고 있다. 이때 인자가 하나인 생성자를 호출하게 된다.
	 */
	 ...
	 * private멤버메소드인 money는 직접 접근할수 없으므로 접근
	...
	public void showAccountMoney() {
		System.out.println("지금까지의 누적액:"+ getAccMoney());
		//System.out.println("지금까지의 누적액:"+ money); //[접근불가]
	}
}
