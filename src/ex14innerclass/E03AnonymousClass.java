...
public class E03AnonymousClass
{
	...
	{
		/*
		 * Student라는 이름을 가진 자식클래스를 통해 객체를 생성하고 부모클래스의 참조변수로 참조함 참조변수가 부모타입이므로 자식객체에 직접
		 * 접근하는것은 불가능. 반드시 다운케스팅 후 접근해야한다.
		 */
		Person person = new Student("이정연", "2016");
		person.printInfo();
		...
		/*
		 * Person클래스를 상속받은 익명클래스 정의 및 생성 :person타입의 인스턴스 변수에 Person을 상속받은 익명클래스를 생성해서 그
		 * 주소를 할당한다. -개념적으로 new Person(){}; ==> 익명 extends Person{}; 형태라 생각하면 된다. -이름이
		 * 없는 객체이므로 부모클래스의 이름을 빌려서 생성했다고 생각하자.
		 */
		Person anonyPerson = new Person("이준승")
		{
			...
			void newMethod()
			{
				System.out.println("익명클래스에서 새롭개 확장한 메소드");
			}
			...
			void printInfo()
			{
				...
				newMethod();
				System.out.println("익명클래스애소 오버라이딩");
			}
		};
...
//		anonyPerson.newMethod(); //[접근불가- 에러발생]
		...
		Student anonyStudent = new Student("이정연1", "1012")
		{
			// 익명클래스애서 확장한 멤버변수
			int age = 1;
...
//			((student)anonyStudent).age = 100;
		...
		/*
		 * 다운케스팅은 "(자식클래스명)부모타입의 참조변수" 형태로 형변환하여 하위클래스의 멤버에 접근하기 위한 수단이다. 그러나 익명클래스는
		 * 클래스명이 없기 때문에 위와같이 다운캐스팅이 불가능하다. 즉, 익명클래스에서는 새롭게 정의한 멤버변수를 접근할수 없다. 따라서 익명클래스는
		 * 오버라이딩의 목적으로만 사용해야한다.
		 */
		/*
		 * Person anonyTest = new Student("다운케스팅", "안되욤") { String jumin = "342435"; };
