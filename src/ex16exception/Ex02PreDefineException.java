...
/*
 ...
 -RuntimeException 클래스 계열로 NullPointException이 대표적인 하위 클래스이다.
 -main메소드애서 무시하기위해 throws하더라도 오류가 발생되므로 반드시 예외처리를 헤줘야 한다.
 예외발생시 JVM은 해당 예외클래스를 인스턴스화 헤서 프로그램쪽으로 전달하게 되고 try~catch에서 받은후 처리한다.
 */
public class Ex02PreDefineException
{
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			intArr[2] =300;/*
							예외가 발생하는 지점으로, 예외객체가 던져지므로 아래 문장은
							실헹되지 않음. catch블럭으로 바로 넘어감.
			 				*/
			...
			/*
			 ...
			 printStackTrace() : 예외가 발생할때의 에러메세지를 그대로 콘솔에 출력해줌. 
			 					개발시 훨씬더 시용빈도 높음
			 */
		...
		System.out.println("==ArrayIndexOutOfBoundException발생후==");
		...
		/*
		 3] InputMismatchException : 숫자를 입력해야 되는데
		 							문자열을 입력할때 발생되는 예외
		 */
		...
		try{
			...
			System.out.println("나이를 문자형태로 입력하면 안되여");
			System.out.println("예외메세지:"+ e.getMessage());
			e.printStackTrace();
		}
		...
		*/
		...
		try {
			...
			System.out.println("형변횐 할수 없습니다,");
