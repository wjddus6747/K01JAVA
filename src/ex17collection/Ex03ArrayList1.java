...
/*
 ArrayList :List계열의 컬랙션으로 
  	-데이터의 중복 저장이 허용된다.
  	-데이터의 저장순서를 보장한다.
  	-데이터에 접근할때 iterator() 혹은 get()을 이용한다
  	-Array라는 이름처럼 '배열'의 특성을 가지고 있어
  	인덱스를 통한 저장및 접근이 가능하다.
 */
public class Ex03ArrayList1
{
	...
	{
		/*
		 * List<E> 인터페이스 계열의 컬랙션
		 * 	: ArrayList<T> 와 같은 컬랙션은 제네릭을 기반으로 하지만
		 * 아래와 같이<T> 부분을 생략하면 모든 객체를 저장할수 있는
		 * 컬랙션이 된다. 즉, <Object> 로 선언한것과 동일한다.
		 */
		...
		System.out.println("중복저장 전 저장된 객체수:"+ list.size());
		 ...
		 */
		System.out.println(list.add("트와이스")?"중복저장됨" : "중복 저장안됨");
		...
		/*
		  Iterator 사용법
		  - 컬랙션에 저장된 내용을 Iterator에게 알려주기 위해 객체를 생성한다
		...
		/*
		 1-4] 덮어쓰기
		 		set(인덱스, 객체) : 특정 인덱스에 저장된 데이터를 덮어쓰게되므로 
		 						기존자료는 삭제된다.
		 */
		list.set(4, "오마이걸 덭어쓰기");
		for(Object obj : list) {
		...
		/*
		 1-5] 끼워넣기
		 		add(삽입할 인덱스, 객체) : 해당 인덱스애 삽입하고, 기존데이터는 뒤로 밀리게 된다.
		 */
		...
		/*
		 1-6] 포함 여부 확인
		 		contains(객체명) : 찾고자 하는 객체가 있는경우 true를 반환한다.
		 */
		...
		/*
		 1-7] 삭제
		 방법1 : 인덱스로 삭제하기
		 		remove(인덱스) : 인덱스를 통해서 삭제를 진행한후 삭제가 완료되면
		 		해당 객체를 반환한다. 삭제 후 에는 인덱스가 자동으로 재부여 된다.
		 */
		Object obj = list.remove(2);
		System.out.println("삭제된 객체:"+ obj);
		
		//방법2 : indexOf메소드로 인덱스를 찾은후 삭제
		int index = list.indexOf("빅뱅");
		System.out.println("빅뱅의 index="+ index);
		list.remove(index);
		
		//방법3 : 인스턴스의 참조값(주소값)을 통해 삭제
		System.out.println(list.remove("방탄소년단")? "삭제성공": "삭제 실패");
		list.remove("오마이걸");
