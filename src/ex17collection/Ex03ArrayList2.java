...
public class Ex03ArrayList2
{
	...
	public static void main(String[] args)
	{
		...
		Student st1 = new Student("이정연", 24, "2020");
		Student st2 = new Student("이준승", 26, "2050");
		Student st3 = new Student("허세진", 20, "2010");
		Student st4 = new Student("공유", 15, "2060");
		...
		*/
		...
		/*
		  인스턴스 삭제 : 인덱스가 아닌 객체의 참조값을 통해 삭재하는 경우
		 			 중복된 값이있으면 앞에 있는 객체 하나만 삭제된다.
		 */
		list2.remove(st2);
		System.out.println("[중복저장된 객체 삭제후");
		for (Student st : list2)
		{
			st.showInfo();
		}
		...
		{/*
			...
			/*
			 위와 동일한 문장임.
			 next()메소드를 통해 객체를 가져온후 showInfo()메소드로 객체릐 내용출력
			 */
			it2.next().showInfo();
		}
		...
		{
			/*
			 리스트컬렉션에 인덱스로 접근할때는 get()메소드를 사용한다.
			 */
