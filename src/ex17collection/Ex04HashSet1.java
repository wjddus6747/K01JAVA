...
/*
 HashSet 컬렉션: Set계열의 인터페이스를 구현한 컬랙션으로
 -객체가 순서없이 저장된다.
 -객체의 중복저장을 기본적으로 허용하지 않는다.
 	단, 기본클래스가 아닌 새롭개 정의한 클래스라면 hashCode,
...
public class Ex04HashSet1
{
	...
	public static void main(String[] args)
	{
		...
		Teacher teacher = new Teacher("이정연", 22, "영어");

		/*
		 * 3]객체저장
		 * 		:add()메소드를 통해 객체가 정상적으로 저장되면 true를 반환한다.
		 */
		...
		/*
		 * 5-1] 기본 클래스형 객체 중복저장
		 * 		:기본클래스의 경우에는 별도의 오버라이딩 없이 중복이 제거된다. 
		 ...
		 * 		저장에 성공하여 true가 반환된다.
		 */
		Teacher teacher2 = new Teacher("이준승", 22, "영어");
		System.out.println(set.add(teacher2) ? "성공" : "실패");
		System.out.println("[중복(techaer2)저장후 객체수]" + set.size());
		...
		//1-8]삭제
		System.out.println(set.remove(strObject2)? "strObject2성공": "strObject2실패");
		System.out.println(set.remove("Android")? "Android성공": "Android실패");
		System.out.println("[삭제후 객체수]"+ set.size());
		
		//1-9] 전체삭제
		//set.clear();
		System.out.println("전체식제:"+ set.removeAll(set));
		System.out.println("[전체식제후 객체수]"+ set.size());
	}
}
