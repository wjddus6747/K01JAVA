...
/*
 ...
 -Set의 기본적인 특성은 동일하나, Tree알고리즘을 통헤 정렬되어
 	저장된다. 정렬의 기준을 개발자가 직접 정의할 수 있다.
 -정령의 기준을 정하기 위헤 Comparable<T>인터페이스를 구현해야한다.
 */
class MyString implements Comparable<MyString>{
	...
	/*
	 정렬의 기준을 위헤 오버라이딩 한 메소드로 가나다와 같은 사전순,
	 문자열의 길이등 다양하게 설정 가능하다.
	...
	public int compareTo(MyString paramStr)
	{
		...
		if(getLegth() > paramStr.getLegth())
			return 1;
		else if(getLegth() < paramStr.getLegth())
			return -1;
		else
			return 0;
	}
...
public class Ex04TreeSetMain
{
	...
	public static void main(String[] args)
	{
		...
		tree1.add(5);
		tree1.add(4);
		System.out.println(tree1.add(2)); // 중복저장을 허용하지 않음
