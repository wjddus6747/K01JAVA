...
/*
 Thread(쓰레드)
 	- 쓰레드의 생성은Thread클래스를 상속받는것부터 시작한다.
 	-해당 방법은 쓰레드로 생성할 클래스가 다른 클래스를 상속받지 않아도 될때 사용하는 방식이다.
...
class ShowThread extends Thread{
	...
	/*
	 ...
	 직접 호출하면 안되고, start()메소드를 통해 간접적으로 호출해야한다.
	 만약 직접 호출하면 단순한 실행만될뿐 쓰레드가 생성되지 않으므로 주위해야한다.
	 */
	@Override
	public void run()
	{
		for(int i=1; i<=100; i++) {
			...
			try {
				/*
				 Thread클래스의 static메소드로 실행의 흐름을 주어진 시간만큼 Block상태로 전환시켜 준다
