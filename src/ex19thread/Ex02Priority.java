...
/*
 쓰레드의 우선순위
 	...
 	getPriority() : 쓰레드의 우선순의를 반환할때 사용.
 */
class MessageSendingThread extends Thread{
	...
	public void run()
	{
		for(int i=1; i<10000; i++) {
			...
			/*
			 기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만, sleep()메소드를 사용하게
			 되면 실행중 블럭상태로전환되어 우선순위가 낮은 쓰레드도 가끔 실행될수 있는 상황이 생기게 된다.
			 */
			try {
				//0.001초
				sleep(1);
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		/*
		 2단계
		 	: 우선순위 부여를 위해 Thread클래스에 static타입의 상수를 이용한다.
		 	 ※우선순의가 높다고 무조건 먼저 실행되지는 않는다.
		 	 전체적인 확률이 높아지게 되어 먼저 실행될 가능성이 높아진다고 생각하면 된다.
		 */
