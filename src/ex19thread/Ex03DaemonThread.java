...
// 쓰레드로 구현하지 않은 클래스
 /*
   :하나의 클래스가 작업을 끝내야 다음 클래스의 작업을 진행할수 있다.
    즉 동시에 작업을 진행할수 없다.
  */
class NotThread
{
...
/*
 * 독립쓰레드(Non DaemonThread)
 * 	: 메인쓰래드와 working쓰래드(개발자가 정의한 쓰래드)
 *  일컫는 것으로 메인쓰레드가 종료되어도 계속 실행되며,
 *  Dead상태가 되어야 실행이 종료되는 쓰래드를 말한다.
 *  해당 쓰래드를 실행하게 되면 main메소드가 종료된 이후에도 실행되는것을 볼 수 있다.
 */
class YesThread extends Thread
{
	...
	/*
	 ...
	 해당 쓰래드의 이름을 지정할수 있다,
	 지정된 이름은 getName()메소드를 통해 반환할수 있다.
	 */
...
public class Ex03DaemonThread
{
	...
	public static void main(String[] args)
	{
		...
		*/
		yt1.setName("첫번째 쓰레드"); //쓰레드의 이름지정 두번째 방법
		yt1.start(); //쓰레드객체를 Runnable상태로 만들어 준다.
		...
		System.out.println("현재활성화된 상태의 쓰레드수"+ Thread.activeCount());
		System.out.println("현재 실행둥인 쓰레드명:"+ Thread.currentThread().getName());
		System.out.println("메인함수의 우선순의:"+ Thread.currentThread().getPriority());
		System.out.println("메인함수의(Main Thread)종료");
	}
